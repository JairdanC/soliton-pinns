...
class PINN(nn.Module):
    ...
    def __init__(self, n_hidden_layers=2, n_neurons_per_layer=9, activation=nn.Tanh, use_layernorm=False):
        ...
        for _ in range(n_hidden_layers):
            ...
            if self.use_layernorm:
                layer_list.append(nn.LayerNorm(n_neurons_per_layer))
            layer_list.append(activation())
...
class PINN3D(nn.Module):
    """
    General PINN class for three inputs (x, y, t) and one output (u).
    """
    def __init__(self, n_hidden_layers=2, n_neurons_per_layer=9, activation=nn.Tanh):
        ...
        # input (x, y, t) and output (u)
        self.n_input_nodes = 3
        self.n_output_nodes = 1
    ...
    def forward(self, x, y, t):
        inputs = torch.cat([x, y, t], dim=1) # Combine inputs into a single tensor
        output = self.model(inputs) # Pass through network layers
        return output
